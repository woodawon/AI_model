...
#전처리함수
def text_process(a):
    morphs_text = mecab.morphs(a) #형태소분석
    
    for word in morphs_text:
        if word not in stopwords: #불용어 제거
            if len(word) > 1: #길이가 1이하인 단어 제거(공백포함)
...
processed_text = text_process(text) #전처리 결과
encoded_text = Integar_Encoding(vocab) #정수 인코딩 결과
